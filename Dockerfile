FROM --platform=linux/amd64 cimg/node:22.22.0 AS aws
...
FROM --platform=linux/amd64 cimg/node:22.22.0
ENV DEBIAN_FRONTEND=noninteractive
...
RUN sudo apt-get update \
   ...
   && sudo apt-get install -y --no-install-recommends libnss3 libatk1.0-0 libatk-bridge2.0-0 libgtk-3.0 libdrm2 libgbm1 libasound2t64 \
   # other dependencies
